...
### 命令词协议表 (16 条, V7 固件 2026-07-27)

**V7 变更**: 播报模式改为被动 — CI1302 仅发送识别帧 (TYPE=0x81), 不再自行播报.
语音输出统一由 M30 + piper-tts 负责.

格式: `命令词 : 识别帧(模块→X5)` (应答帧保留但 X5 不再发送)
...
- 版本: CI1302 V7 (2026-07-27, 基于 V01843 SDK, 内部RC + 关闭波特率校准, **被动播报模式**)
- 唤醒词: 独立 DNN 模型门控 (`USE_SEPARATE_WAKEUP_EN=1`)
- 文件: `ci1302_firmware/sfw20260727161528158195790/`
- 播报: V7 起模块不自播, 所有语音反馈由 M30 + piper-tts 统一输出
