...
|------|------|---------------|
| CH1 | 转向 | channels[0] |
| CH2 | 升降 | channels[1] (未用) |
| CH3 | 油门 | channels[2] |
| CH4 | 方向 | channels[3] (未用) |
| CH5 | ARM/DISARM | channels[4] (> 1024 = ARM) |
| CH6 | MODE | channels[5] (预留) |
...
```
SBUS 遥控器 (有效 + CH5=ARMED)  ▸  最高优先
X5 自主指令 (SBUS 断开或 DISARMED) ▸  次优先
超时刹停 (60s 无任何命令)         ▸  安全兜底
```

### Failsafe

SBUS 帧 byte23 bit3=1 → 接收机失联 → STM32 立即切中位 1275μs
SBUS 100ms 无有效帧 → 视为断开 → 自动回退到 X5 模式
