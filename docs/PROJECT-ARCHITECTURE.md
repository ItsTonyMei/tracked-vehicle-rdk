...
**重型履带车 RDK X5 自主跟随系统** — 基于地平线 RDK X5 开发板的 ROS2 机器人，通过语音+手势控制实现对特定人物的自动跟随。
...
| 车体 | 重型履带车 | ZTW Seal G2 ESC, 1500μs 中位 PWM |
...
2. **手势锁定**: OK=11/👍=2 并行锁定 + Palm=5 解锁, 时间窗口投票 (1.0s / ≥15 命中) + 人物多数票表决 + 最佳距离匹配
...
**FOLLOWING 模式**: 复合控制:
- **线速度**: LiDAR 融合距离 → 连续速度映射 + LPF 平滑 + 加速度限制 (0.8 m/s²)
  - <0.5m: -0.3 m/s (全速后退)
  - 0.5-0.85m: -0.3→0 分段过渡 (min(vel, floor) 地板 -0.15 克服静摩擦)
  - 0.85-1.0m: 0 (Schmitt 迟滞退出, 不产生正向速度)
  - 1.0-1.2m: 0 (停止区)
  - 1.2-3.0m: 0→0.8 m/s (二次曲线加速)
  - ≥3.0m: 0.8 m/s (全速)
  - EKF vx 前馈 (LPF α=0.20): 人在靠近时平滑增加后退量
- **角速度**: LiDAR 侧向偏移 P+LPF (k=0.4, deadband ±5cm, α=0.25) 或 bbox (fallback)
...
| ★ 后退平滑 | Schmitt 迟滞 + min(vel, floor) 地板 + EKF vx 前馈 LPF + 线速度 LPF + 加速度限制 |
