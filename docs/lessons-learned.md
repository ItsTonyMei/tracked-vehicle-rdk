...
### 43. 手势滑动窗口 code=0 污染 — 投票被稀释 3-5x

- **症状**: 多人同框下手势锁定需要 2-3 秒, 有时更长
- **根因**: `gesture_cb` 中 `if code != 0 and score < threshold: continue` 逻辑错误 — code=0 时 `0 != 0` 为 False, 整个条件短路, 零条目从不跳过。hand_gesture_detection @ 60fps 将 30 帧窗口填满 (0, score), 真实手势投票被稀释到 10-20% 占比
- **修复**: `code == 0 or score < threshold` → 仅非零码进入窗口; 时间窗口 (1.0s) 替代帧计数, 旧投票自动过期
- **教训**: 逻辑与 (AND) 短路求值的 bug 极难在 code review 中发现, 因为读代码时 "code != 0" 读起来像 "跳过非零码的低置信度", 但实际语义是 "仅在非零时检查置信度"。条件中正/负语义与短路组合时需格外谨慎

### 44. max/min 混淆 — 后退地板变成天花板

- **症状**: 后退速度始终不超 -0.15 m/s (不足克服静摩擦), EKF 前馈触发时突然加速到 -0.45
- **根因**: `max(vel, -0.15)` 对负数语义反转 — max 取更接近零的值, 将 -0.3 截为 -0.15。注释写 "不低于 floor" 但 max 实现的是 "不高于 floor" (天花板)。同时 EKF 前馈无 LPF, vx 跳变直接放大灌入
- **修复**: `min(vel, floor)` + `min(vel, 0.0)` 防迟滞区正向 + EKF 前馈 LPF (α=0.20) + 输出端 LPF (α=0.30) + 加速度限制 (0.8 m/s²)
- **教训**: 负数域的 min/max 与直觉相反 — "地板" (下限, 更负) 用 min, "天花板" (上限, 更近零) 用 max。团队 code review 时应对含负数的边界检查做显式值域测试

### 45. 多人场景空间匹配 — 单帧最近 ≠ 做手势的人

- **症状**: 多人同框下, 做 OK 手势的是 A, 但系统锁定了 B
- **根因**: (1) Pass 1 手在人体框内时返回第一个匹配 (dict 迭代序), 不比较距离; (2) Pass 2 伸臂场景手在框外, 手点到 B 的身体中心比到 A 的更近; (3) 触发时仅用单帧匹配, 缺乏时序一致性
- **修复**: Pass 1 遍历所有命中收集距离, 返回最近; 投票窗口存储每帧匹配的 person_id, 触发时 Counter 多数票表决; `_on_ok` 增加画面内验证
- **教训**: 空间匹配不能只看一帧 — 手势识别本身就是间歇性的, 投票期间每一帧都应匹配, 最终用统计方法 (多数票) 决定

### 46. 避障与跟随耦合 — 反应式避障引入连锁问题 (v0.11.x 已回退)
- **症状**: 跟随中避障 (三扇区/软停绕障/转向避让) 在实车测试中反复出现蛇行、卡死、
  失控旋转、丢跟随 — 从 v0.11.0 到 v0.11.11 连续 8 轮修复仍无法可靠工作
- **根因** (按严重度):
  1. 避让转向与 PD 追人方向冲突 → 净转向不足 → 车在减速区蹭着绕不开
  2. 软停纯零速/闩锁无恢复路径 → 贴脸障碍永久卡死
  3. 墙/长障碍绕障无退出机制 → 失控旋转
  4. 被锁人豁免一刀切角度 → 柱子被豁免 → 顶柱; 相反方向手臂点触发避障 → 蛇行
  5. 扇区统计边界与预过滤不一致等多次"定义改了逻辑没同步"的低级 bug
- **修复**: v0.12.0 回退全部避障逻辑到 v0.10.1 基线, 仅保留手势锁定增强
- **教训**: 移动机器人"跟随 + 避障"本质是路径规划问题, 不是多层 if-else 反应式叠加;
  反应式规则 (扇区/阈值/方向) 在真实场景的几何多样性下必然出现组合爆炸;
  正确路线是局部代价地图 + 规划器 (如 Follow the Gap / TEB), 或用纯反应式但
  **严格解耦** (避障独立于跟随输出, 不叠加不衰减 PD); 任何"方向/边界"规则改动
  必须配单测 (注入角度断言方向) — 本系列 3 次"定义改了逻辑没同步"的回归即由此而来
